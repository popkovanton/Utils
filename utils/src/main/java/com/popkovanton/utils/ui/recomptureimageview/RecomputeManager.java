...
public class RecomputeManager {
    private Size mViewSize;
    private Size mImageSize;

    public RecomputeManager(Size viewSize, Size imageSize) {
        mViewSize = viewSize;
    ...
    private Matrix startInside() {
        if (mImageSize.getHeight() <= mViewSize.getWidth()
            ...
            // image is smaller than view size
            return getOriginalScale(PivotPoint.LEFT_TOP);
        } else {
            // either of width or height of the image is larger than view size
            return fitStart();
        }
    }
